...
const ScreenController = function () {
  ...
  const loadTasks = (selectedList) => {
    ...
    const tasks = tasksFromStorage
      ? tasksFromStorage
      : TaskController.getTasks(selectedList.id);
    tasks.forEach((task) => {
  ...
  const taskWindowTemplate = () => {
    const listName = document.getElementById('content-heading').innerText;
    ...
    taskWindow.innerHTML = `
        ...
        <label id="task-list-label" class="task-input-label">List: 
